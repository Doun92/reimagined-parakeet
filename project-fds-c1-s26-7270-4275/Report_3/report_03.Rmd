...
# Introduction
This is the third report in the series of four reports.
This report focus on data analytics using APIs.
This report is a good opportunity to train on reading API documentation and getting the data in a meaningful way.

This report will focus on movies analytics from the Move Database, a website that has as a mission to list all the movies created.

They Movie Database describes itself as:

<blockquote>
The Movie Database (TMDB) is a community built movie and TV database. Every piece of data has been added by our amazing community dating back to 2008. TMDB's strong international focus and breadth of data is largely unmatched and something we're incredibly proud of. Put simply, we live and breathe community and that's precisely what makes us different.
</blockquote>

In this report we will extract information from their API and get some meaningful analytics.

# The Data
The data comes from the https://www.themoviedb.org/.
As the data comes from API calls, we receive it as a JSON scheme. It means the data is not given to us as a table, not in a hierarchical way.

We will mainly focus on the table `movies`, and use the other tables as helpers to make our data more understandable. In the beginning, we will play a bit with other tables to be in shape.
...
We also can determine that Pixar movies are more likely to be popular than Disney movies. The aquisition of Pixar is more to get two times more movies in the cinemas than just one.

# Conclusion
In this project we went throught The Movie Database API.

We then delved deeper into the analytics by examining movies produced by Pixar, a renowned animation studio, and comparing them to those produced by Disney. Utilizing the API to gather movie information, we conducted a comparative analysis of the popularity of Disney and Pixar films released from 2006 onwards. Our findings revealed interesting trends in audience reception and provided valuable insights into the strategic implications of Disney's acquisition of Pixar.

With the spirit of always improving, we could also think about <i>caching</i> the data in to csv files. So we could avoid too many API calls. Also, we could think about a table before doing the APIs and then fill it to answer our questions.
